...
uses
  ...
  JvFullColorSpaces, JvFullColorCtrls, Menus, JvMenus;
...
type
  TForm1 = class(TForm)
    ...
    procedure combo_palettePropertiesCloseUp(Sender: TObject);
    procedure combo_palettePropertiesChange(Sender: TObject);
    procedure combo_paletteClick(Sender: TObject);
  private
...
procedure TForm1.doWritePaletteToCombo;
var
      namePT : string;
      i:integer;
      FInfo : TSearchRec;
...
begin
      ...
      begin
            ...
            combo_palette.Properties.Items.Add(s);
...
begin
     ...
     begin
            s := FInfo.Name;
            delete(s,length(s)-length(ExtractFileExt(s))+1 ,length(ExtractFileExt(s)));
            if s <> '' then hasPaletteFile := hasPaletteFile + 1;
            aResult := FindNext(FInfo);
     end;

     if hasPaletteFile > 0 then
     begin
        doWritePaletteToCombo;
        for i:= 0 to combo_palette.Properties.Items.Count-1 do
        begin
            if combo_palette.Properties.Items.Strings[i] = txt_fileName then
            begin
                  combo_palette.ItemIndex := i;
                  readLineNum := 0;
                  AssignFile(myPTfile,(ExtractFilePath(Application.ExeName)+'palettes\'+txt_fileName+'.dat'));
                  Reset(myPTfile);
                  repeat
                  ...
                  if readLineNum = 0 then caption := 'nothing';
     ...
     begin
        ...
        newG := 0;
        newB := 0;
        combo_palette.Properties.Items.Add('untitled');
        combo_palette.ItemIndex := combo_palette.Properties.Items.Count-1;
        isSaved := False;
     end;
...
begin
      ...
      if hasPaletteFile = 0 then
      begin
            writeFileName := combo_palette.Properties.items.Strings[combo_palette.Properties.Items.Count-1];
      end else
      begin
            writeFileName := combo_palette.Properties.Items.Strings[combo_palette.ItemIndex];
      end;
      AssignFile(infoT,ExtractFilePath(Application.ExeName)+'info.dat');
      ReWrite(infoT);
      WriteLn(infoT,IntToStr(newR)+chr(9)+IntToStr(newG)+chr(9)+IntToStr(newB));
      WriteLn(infoT,writeFileName);
      closeFile(infoT);
      if isSaved = False then mu_saveClick(Sender);
end;
...
begin
      if isSaved then mu_save.Enabled := False else mu_save.Enabled := True;
      if hasPaletteFile = 0 then
      begin
            mu_delete.Enabled := False;
            mu_saveas.Enabled := False;
            mu_rename.Enabled := False;
      end else
      begin
            mu_delete.Enabled := True;
            mu_saveas.Enabled := True;
            mu_rename.Enabled := True;
      end;
...
var
      buttonSelect,i,selTodel : integer;
      FInfo : TSearchRec;
      aResult: integer;
      ss : array[0..255] of  char;
      s : string;
begin
      //save the existing palette;
      AssignFile(myPTfile,(ExtractFilePath(Application.ExeName)+'palettes\'+combo_palette.Properties.Items.Strings[combo_palette.ItemIndex]+'.dat'));
      ReWrite(myPTfile);
      for i:=0 to high(ppPalette) do
      begin
      ...
      isSaved := True;
      // deal with delete ....
      buttonSelect := MessageDlg('The palette can not be restored after you delete it.'+#13+#10+'Do you want to continue ?', mtWarning, [mbYes,mbNo], 0);
      if buttonSelect = mrYes then
      begin
            {
            1. setPalette length = 0;
            2. clean the myPalette;
            3. delete the file in the folder;
            4. delete the item in combobox;
            6. check if any palette file left, otherwise, sel hasPaletteFile := 0(in call newppClick);
            }
            selTodel := combo_palette.ItemIndex;
            if deleteFile(ExtractFilePath(Application.ExeName)+'palettes\'+combo_palette.Properties.Items.Strings[selTodel]+'.dat') then
            begin
                  combo_palette.Properties.Items.Delete(selTodel);
                  ...
                  if hasPaletteFile = 0 then
                  begin
                        mu_newppClick(Sender);
                  end else
                  begin
                        combo_palette.ItemIndex := combo_palette.Properties.Items.Count-1;
                        combo_palettePropertiesCloseUp(Sender);
                  end;
            end else ShowMessage('Palette is not deleted, error = '+
...
begin
      ...
      begin
            showMessage('You have not saved this palette yet!');
      end;
end;
...
procedure TForm1.combo_palettePropertiesCloseUp(Sender: TObject);
var
      readLineNum,i2,j,y,k : integer;
      readColorStr : string;
      oldFileName : string;
begin
    ...
    begin
       //caption := oldFileName;
      {oldFileName := combo_palette.Properties.Items.Strings[combo_palette.ItemIndex];
      AssignFile(myPTfile,(ExtractFilePath(Application.ExeName)+'palettes\'+combo_palette.Properties.Items.Strings[combo_palette.ItemIndex]+'.dat'));
      ReWrite(myPTfile);
      for k:=0 to high(ppPalette) do
      begin
                WriteLn(myPTfile,(IntToStr(ppPalette[k,0])+chr(9)+IntToStr(ppPalette[k,1])+chr(9)+IntToStr(ppPalette[k,2])));
      end;
      closeFile(myPTfile);
      isSaved := True;}
    end else
...
procedure TForm1.combo_palettePropertiesChange(Sender: TObject);
begin
      oldFileName := combo_palette.Text;
end;

procedure TForm1.combo_paletteClick(Sender: TObject);
begin
      caption := oldFileName;
      //if isSaved = False then mu_saveClick(Sender); 
end;
