...
begin
  saveToSave := 0;
  StrPCopy(ss,Form1.paletteFolderPath+'\*.dat');
  ...
  begin
      AssignFile(Form1.myPTfile,(Form1.paletteFolderPath+'\'+pt_filename.Text+'.dat'));
      ...
      form1.main_status.Panels[0].Text := 'Saved file';
      form1.main_status.Panels[1].Text := pt_filename.Text+'(Saved)';
      
      Form4.Close;
  end else
  begin
      buttonSelect := MessageDlg('Palette: '+pt_filename.Text+' - is already existed.'+#13+#10+'Do you want to overwrite it? ', mtWarning, [mbYes,mbNo], 0);
      if buttonSelect = mrYes then
      begin
              //do overwrite file
              DeleteFile(Form1.paletteFolderPath+'\'+pt_filename.Text+'.dat');
              DeleteFile(Form1.paletteFolderPath+'\'+Form1.oldFileName+'.dat');
              AssignFile(Form1.myPTfile,(Form1.paletteFolderPath+'\'+pt_filename.Text+'.dat'));
        ReWrite(Form1.myPTfile);
        for i:=0 to high(Form1.ppPalette) do
        begin
      ...
      //closeFile(Form1.infoT);
        ...
        form1.pp_pageName.Caption := pt_filename.Text;
        form1.main_status.Panels[0].Text := 'Overwrited file';
        form1.main_status.Panels[1].Text := pt_filename.Text+'(Saved)';
