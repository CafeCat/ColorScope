...
uses
  ...
  TFlatButtonUnit, TFlatSpinEditUnit, JvToolBar, ToolWin, JvCoolBar,
  TFlatGroupBoxUnit;
...
begin
      ...
      spBMP.LoadFromFile('sample.bmp');
      spBMP.TransparentColor := RGB(255,0,255);
      spBMP.Transparent := TRUE;
      ...
      with Color_model1 do begin // draw a sampler with 9pixelx9pixel
          ...
          canvas.LineTo((modelAtX-4),(modelAtY+4));
          canvas.LineTo((modelAtX-4),(modelAtY-4));}
          //Canvas.Rectangle(bounds(modelAtX-4,modelAtY-4,9,9))
          canvas.Draw(modelAtX-5,modelAtY-5,spBMP);
      end;
      spBMP.Destroy;
...
begin
      ...
      begin
            modelAtX := X;
            if X > 219 then modelAtX := 219;
            if X < 0 then modelAtX := 0;
            modelAtY := Y;
            if Y > 219 then modelAtY := 219;
            if Y < 0 then modelAtY := 0;
            Color_model1Paint(Sender);
      end;
end;
...
begin
      ...
      begin
          ...
          Color_mixer.Canvas.MoveTo(0,16*j+1);
          Color_mixer.Canvas.Rectangle(bounds(0,16*j+(j*1),16,16));
      end;
end;
...
begin
      ...
      for i := 0 to 11 do
      begin
            if (Y < (mixerColor[i,3]+16)) AND ( Y > mixerColor[i,3]) then
            begin
                  isFromInput := False;
                  found := true;
                  newR := mixerColor[i,0];
...
begin
    ...
    begin
            ...
            Canvas.Rectangle(bounds(0,0,30,30));
    end;
end;
